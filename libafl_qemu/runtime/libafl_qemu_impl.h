...
noinline libafl_word libafl_qemu_start_virt(void *buf_vaddr, libafl_word max_len)
{
  return _libafl_sync_exit_call2(LIBAFL_QEMU_COMMAND_START_VIRT,
                                 (libafl_word)buf_vaddr, max_len);
}

noinline libafl_word libafl_qemu_start_phys(void *buf_paddr, libafl_word max_len)
{
  return _libafl_sync_exit_call2(LIBAFL_QEMU_COMMAND_START_PHYS,
                                 (libafl_word)buf_paddr, max_len);
}

noinline libafl_word libafl_qemu_input_virt(void *buf_vaddr, libafl_word max_len)
{
  return _libafl_sync_exit_call2(LIBAFL_QEMU_COMMAND_INPUT_VIRT,
                                 (libafl_word)buf_vaddr, max_len);
}

noinline libafl_word libafl_qemu_input_phys(void *buf_paddr, libafl_word max_len)
{
...
noinline void libafl_qemu_trace_vaddr_range(libafl_word start, libafl_word end)
{
    _libafl_sync_exit_call2(LIBAFL_QEMU_COMMAND_VADDR_FILTER_ALLOW, start, end);
}

noinline void libafl_qemu_trace_vaddr_size(libafl_word start, libafl_word size)
{

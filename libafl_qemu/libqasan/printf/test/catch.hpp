...
unsigned int rngSeed();
  ...
  // Cygwin
    ...
    #define _BSD_SOURCE
    // some versions of cygwin (most) do not support std::to_string. Use the
    // libstd check.
...
struct is_unique<T0, T1, Rest...>
  ...
  // each invocation, e.g. CATCH_REC_LIST_UD(f, x, a, b, c) evaluates to f(x,
  // a), f(x, b), f(x, c)
  ...
  #define INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME2(Name, ...) \
    ...
    #define INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME1(Name, ...) \
      INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME2(Name, __VA_ARGS__)
    #define INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME(Name, ...) \
      INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME1(                \
          Name, INTERNAL_CATCH_EXPAND_VARGS(               \
                    INTERNAL_CATCH_REMOVE_PARENS(__VA_ARGS__)))
  #endif

  #define INTERNAL_CATCH_MAKE_TYPE_LIST(types) \
    Catch::TypeList<INTERNAL_CATCH_REMOVE_PARENS(types)>

  #define INTERNAL_CATCH_MAKE_TYPE_LISTS_FROM_TYPES(types) \
    CATCH_REC_LIST(INTERNAL_CATCH_MAKE_TYPE_LIST,          \
                   INTERNAL_CATCH_REMOVE_PARENS(types))
...
struct rewrap<Container, List<Elems...>, Elements...> {
  using type =
      typename append<TypeList<Container<Elems...>>,
                      typename rewrap<Container, Elements...>::type>::type;
};
    ...
    struct into {
      using type =
          typename append<Final<>,
                          typename rewrap<Containers, Types...>::type...>::type;
    };
  };
};
...
struct AutoReg : NonCopyable {
  ...
  ~AutoReg();
    ...
    #define INTERNAL_CATCH_TESTCASE_METHOD_NO_REGISTRATION(TestName,       \
                                                           ClassName, ...) \
      namespace {                                                          \
  ...
  #define INTERNAL_CATCH_TEMPLATE_TEST_CASE_2(TestName, TestFunc, Name, Tags, \
    ...
    #define CATCH_INTERNAL_CHECK_UNIQUE_TYPES(...) \
      static_assert(                               \
          Catch::is_unique<__VA_ARGS__>,           \
          "Duplicate type detected in declaration of template test case");
  #else
    #define CATCH_INTERNAL_CHECK_UNIQUE_TYPES(...) \
      static_assert(                               \
          Catch::is_unique<__VA_ARGS__>::value,    \
          "Duplicate type detected in declaration of template test case");
  #endif
  ...
  #define INTERNAL_CATCH_TEMPLATE_REGISTRY_INITIATE(TestName, Name, ...)   \
    static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {        \
      TestName<CATCH_REC_LIST(INTERNAL_CATCH_REMOVE_PARENS, __VA_ARGS__)>( \
          CATCH_REC_LIST_UD(INTERNAL_CATCH_TEMPLATE_UNIQUE_NAME, Name,     \
                            __VA_ARGS__));                                 \
      return 0;                                                            \
    }();

  #define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE2(                          \
      TestName, TestFuncName, Name, Tags, TmplTypes, TypesList)                \
    CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS                                   \
    template <typename TestType>                                               \
    ...
    static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {            \
      using TestInit =                                                         \
          Catch::combine<INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>::with_types< \
              INTERNAL_CATCH_MAKE_TYPE_LISTS_FROM_TYPES(                       \
                  TypesList)>::into<TestName>::type;                           \
      TestInit();                                                              \
      return 0;                                                                \
    }();                                                                       \
    }                                                                          \
  ...
  #define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_2(                       \
      TestNameClass, TestName, ClassName, Name, Tags, ...)                  \
    CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS                                \
    namespace {                                                             \
    template <typename TestType>                                            \
    struct TestName : INTERNAL_CATCH_REMOVE_PARENS(ClassName<TestType>) {   \
    ...
    INTERNAL_CATCH_TEMPLATE_REGISTRY_INITIATE(TestNameClass, Name,          \
                                              __VA_ARGS__)                  \
    }                                                                       \
    CATCH_INTERNAL_UNSUPPRESS_GLOBALS_WARNINGS                              \
    template <typename TestType>                                            \
  ...
  #define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE_METHOD_2(                  \
      TestNameClass, TestName, ClassName, Name, Tags, TmplTypes, TypesList)    \
    CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS                                   \
    template <typename TestType>                                               \
    struct TestName : INTERNAL_CATCH_REMOVE_PARENS(ClassName<TestType>) {      \
      void test();                                                             \
    ...
    static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {            \
      using TestInit =                                                         \
          Catch::combine<INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>::with_types< \
              INTERNAL_CATCH_MAKE_TYPE_LISTS_FROM_TYPES(                       \
                  TypesList)>::into<TestNameClass>::type;                      \
      TestInit();                                                              \
      return 0;                                                                \
    }();                                                                       \
    }                                                                          \
...
struct StringMaker<
  ...
  #include <iosfwd>
    ...
    #pragma warning( \
        disable : 4312)  // Converting int to T* using reinterpret_cast (issue
                         // on x64 platform)
    #pragma warning( \
 ...
 public:
  ...
  // end catch_message.h
   ...
   // short-circuit evaluation if the type has overloaded &&.
    ...
    ///////////////////////////////////////////////////////////////////////////////
    #define INTERNAL_CATCH_THROWS_AS(macroName, exceptionType,            \
...
struct UnorderedEqualsMatcher : MatcherBase<std::vector<T>> {
  ...
  bool match(std::vector<T> const &vec) const override {
    ...
    if (lfirst == llast) { return true; }

    for (auto mid = lfirst; mid != llast; ++mid) {
      // Skip already counted items
      if (Detail::contains(lfirst, mid, *mid)) { continue; }
...
GeneratorWrapper<T> map(Func &&function, GeneratorWrapper<T> &&generator) {
  return GeneratorWrapper<T>(pf::make_unique<MapGenerator<T, T, Func>>(
      std::forward<Func>(function), std::move(generator)));
}
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
  ...
  static GeneratorTracker &acquire(
    ...
    if (!ctx.completedCycle() && !tracker->isComplete()) { tracker->open(); }
...
// end catch_default_main.hpp
  ...
  #if !defined(CATCH_CONFIG_DISABLE)
    //////
    // If this config identifier is defined then all CATCH macros are prefixed
    // with CATCH_
      ...
      #define CATCH_FAIL_CHECK(...)                                \
        INTERNAL_CATCH_MSG(                                        \
            "CATCH_FAIL_CHECK", Catch::ResultWas::ExplicitFailure, \
    ...
    // required
      ...
      #define REQUIRE_THROWS_WITH(expr, matcher)                            \
        ...
        #define REQUIRE_THROWS_MATCHES(expr, exceptionType, matcher) \
          INTERNAL_CATCH_THROWS_MATCHES(                             \
              "REQUIRE_THROWS_MATCHES", exceptionType,               \
              Catch::ResultDisposition::Normal, matcher, expr)
      #endif  // CATCH_CONFIG_DISABLE_MATCHERS

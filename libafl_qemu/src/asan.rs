...
pub fn init_with_asan(
...
) -> Result<(&'static Emulator, Pin<Box<AsanGiovese>>), EmuError> {
    ...
    let emu = Emulator::init(args, env)?;
...
impl QemuAsanHelper {
    ...
    pub fn new(
        ...
        assert!(unsafe { ASAN_INITED }, "The ASan runtime is not initialized, use init_with_asan(...) instead of just Emulator::init(...)");

...
impl<S, M: EmulatorModule<S>> EmulatorModule<S> for IfModule<M> {
    type ModuleAddressFilter = M::ModuleAddressFilter;

    #[inline]
    fn address_filter(&self) -> &Self::ModuleAddressFilter {
        self.inner.address_filter()
    }

    #[inline]
    fn address_filter_mut(&mut self) -> &mut Self::ModuleAddressFilter {
        self.inner.address_filter_mut()
    }
    ...
    {
        if self.enabled {
            self.inner.init_module(emulator_modules)
        }
    }

    #[inline]
    fn first_exec<ET>(&mut self, emulator_modules: &mut EmulatorModules<ET, S>, state: &mut S)
    where
        ET: EmulatorModuleTuple<S>,
    {
        if self.enabled {
            self.inner.first_exec(emulator_modules, state);
        }
    }
    ...
    {
        if self.enabled {
            self.inner.pre_exec(emulator_modules, state, input);
        }
    }
    ...
    #[inline]
    fn update_address_filter(&mut self, qemu: Qemu, filter: Self::ModuleAddressFilter) {
        self.inner.update_address_filter(qemu, filter);
    }
}

    ...
    pub fn reset(&mut self, qemu: Qemu) {
        {
            ...
            if new_brk < self.brk {
                ...
                let new_page_boundary = (new_brk + ((SNAPSHOT_PAGE_MASK - 1) as GuestAddr)) & (!(SNAPSHOT_PAGE_SIZE -1) as GuestAddr);
                let old_page_boundary = (self.brk + ((SNAPSHOT_PAGE_MASK - 1) as GuestAddr)) & (!(SNAPSHOT_PAGE_SIZE -1) as GuestAddr);

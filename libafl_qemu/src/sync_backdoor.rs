...
use crate::{Emulator, GuestAddrKind, GuestPhysAddr, GuestReg, GuestVirtAddr, Regs, SYNC_BACKDOOR_ARCH_REGS};
...
impl Display for CommandInput {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{} ({} max nb bytes)", self.addr, self.max_input_size)
    }
}
...
impl TryFrom<&Emulator> for SyncBackdoor {
    ...
    fn try_from(emu: &Emulator) -> Result<Self, Self::Error> {
        ...
        let cmd_id: GuestReg = emu.read_reg::<Regs, GuestReg>(arch_regs_map[SyncBackdoorArgs::Cmd])?;

        Ok(match cmd_id.try_into()? {
            NativeSyncBackdoorCommand::Save => {
                SyncBackdoor {
                    command: Command::Save,
                    arch_regs_map
                }
            }
            NativeSyncBackdoorCommand::Load => {
                SyncBackdoor {
            ...
            NativeSyncBackdoorCommand::InputVirt => {
                let virt_addr: GuestVirtAddr = emu.read_reg(arch_regs_map[SyncBackdoorArgs::Arg1])?;
                let max_input_size: u64 = emu.read_reg(arch_regs_map[SyncBackdoorArgs::Arg2])?;
            ...
            NativeSyncBackdoorCommand::InputPhys => {
                let phys_addr: GuestPhysAddr = emu.read_reg(arch_regs_map[SyncBackdoorArgs::Arg1])?;
                let max_input_size: u64 = emu.read_reg(arch_regs_map[SyncBackdoorArgs::Arg2])?;
            ...
            NativeSyncBackdoorCommand::End => {
                let native_exit_kind: GuestReg = emu.read_reg(arch_regs_map[SyncBackdoorArgs::Arg1])?;

...
/// The [`FridaInProcessExecutorMut`] is an [`Executor`] that executes the target in the
...
pub struct FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
...
impl<'a, 'b, 'c, H, OT, RT, S> Debug for FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
    H: FnMut(&mut S::Input) -> ExitKind,
    S: UsesInput,
    S::Input: HasTargetBytes,
...
{
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        f.debug_struct("FridaInProcessExecutorMut")
            .field("base", &self.base)
            .field("helper", &self.helper)
            .field("followed", &self.followed)
            .finish_non_exhaustive()
...
impl<'a, 'b, 'c, EM, H, OT, RT, S, Z> Executor<EM, Z>
    for FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
    EM: UsesState<State = S>,
    H: FnMut(&mut S::Input) -> ExitKind,
    S: UsesInput,
...
impl<'a, 'b, 'c, H, OT, RT, S> UsesObservers for FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
    H: FnMut(&mut S::Input) -> ExitKind,
    OT: ObserversTuple<S>,
    S: UsesInput,
...
impl<'a, 'b, 'c, H, OT, RT, S> UsesState for FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
    H: FnMut(&mut S::Input) -> ExitKind,
    OT: ObserversTuple<S>,
    S: UsesInput,
...
impl<'a, 'b, 'c, H, OT, RT, S> HasObservers for FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
    H: FnMut(&mut S::Input) -> ExitKind,
    S::Input: HasTargetBytes,
    S: UsesInput,
...
impl<'a, 'b, 'c, H, OT, S, RT> FridaInProcessExecutorMut<'a, 'b, 'c, H, OT, RT, S>
where
    H: FnMut(&mut S::Input) -> ExitKind,
    S: UsesInput,
    S::Input: HasTargetBytes,

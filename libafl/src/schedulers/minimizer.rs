...
where
    CS: Scheduler,
    F: TestcaseScore,
    M: AsSlice<Item = usize> + SerdeAny + HasRefCnt,
    Self::State: HasCorpus + HasMetadata + HasRand,
{
    ...
    fn on_remove(
        ...
        state: &mut Self::State,
        idx: usize,
        testcase: &Option<Testcase<Self::Input>>,
    ) -> Result<(), Error> {
...
where
    ...
    F: TestcaseScore,
    M: AsSlice<Item = usize> + SerdeAny + HasRefCnt,
    <Self as Scheduler>::State: HasCorpus + HasMetadata + HasRand,
{
    ...
    #[allow(clippy::unused_self)]
    pub fn cull(&self, state: &mut <Self as Scheduler>::State) -> Result<(), Error> {
...
pub type LenTimeMinimizerScheduler<CS, M> = MinimizerScheduler<CS, LenTimeMulTestcaseScore, M>;

/// A [`MinimizerScheduler`] with [`LenTimeMulTestcaseScore`] to prioritize quick and small [`Testcase`]`s`
/// that exercise all the entries registered in the [`MapIndexesMetadata`].
pub type IndexesLenTimeMinimizerScheduler<CS> =
    MinimizerScheduler<CS, LenTimeMulTestcaseScore, MapIndexesMetadata>;

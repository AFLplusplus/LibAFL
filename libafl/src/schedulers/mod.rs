...
pub use probabilistic_sampling::*;
...
pub fn on_evaluation_metadata<CS, O, OT, S>(
    scheduler: &mut CS,
    state: &mut S,
    observers: &OT,
) -> Result<(), Error>
...
pub fn on_next_metadata<S>(state: &mut S) -> Result<(), Error>
where
    S: HasTestcase,
{
    let current_id = *state.corpus().current();
...
pub trait Scheduler<I, OT, S> {
    /// Called when a [`Testcase`] is added to the corpus
    fn on_add(&mut self, _state: &mut S, _id: CorpusId) -> Result<(), Error>;
    // Add parent_id here if it has no inner

    /// An input has been evaluated
    fn on_evaluation(&mut self, _state: &mut S, _input: &I, _observers: &OT) -> Result<(), Error> {
        Ok(())
    }
...
impl<I, OT, S> Scheduler<I, OT, S> for RandScheduler
where

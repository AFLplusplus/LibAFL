...
impl<F> Scheduler for ProbabilitySamplingScheduler<F>
where
    F: TestcaseScore,
...
mod tests {
    ...
    impl TestcaseScore for UniformDistribution
    where
        Self::State: HasMetadata + HasCorpus,
    {
        fn compute(_: &mut Testcase<Self::Input>, _state: &Self::State) -> Result<f64, Error> {
            Ok(FACTOR)
        }
    }

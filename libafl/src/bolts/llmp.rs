...
use crate::{
    bolts::shmem::{RcShMemProvider, ShMem, ShMemDescription, ShMemId, ShMemProvider},
    Error,
};
...
unsafe fn _llmp_page_init<SHM: ShMem>(shmem: &mut SHM, sender: u32, allow_reinit: bool) {
    ...
    (*page).shmem_id = *shmem.id().as_slice();
    (*page).sender = sender;
    ...
    SP: ShMemProvider,
{
    ...
    pub fn on_port(mut shmem_provider: SP, port: u16) -> Result<Self, Error> {
            ...
            match (*msg).tag {
                ...
                LLMP_TAG_END_OF_PAGE => {
                    ...
                    self.current_recv_map = LlmpSharedMap::existing(
                        self.shmem_provider.from_id_and_size(

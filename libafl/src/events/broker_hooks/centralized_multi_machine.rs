...
#[derive(Debug)]
pub struct TcpMultiMachineLlmpHook<A, I>
where
    I: Input,
{
...
impl<A, I> TcpMultiMachineLlmpHook<A, I>
where
...
impl<A, I, SP> LlmpHook<SP> for TcpMultiMachineLlmpHook<A, I>
where
    ...
    I: Input + Send + Sync + 'static,
{
    ...
    fn on_new_message(
        ...
        TcpMultiMachineState::init_once(shared_state.clone(), self.rt.clone().unwrap().clone())?;

        info!("Using shared state {:?}", shared_state);

        if *msg_tag == _LLMP_TAG_TO_MAIN {
            // From client to main, we inject external events
            info!("From client to main");
            ...
            let res: Result<(), Error> = self.rt.clone().unwrap().block_on(async move {
                ...
                state_wr_lock
                    .handle_new_messages_from_nodes(&mut new_events)
                    .await?;
            ...
            // From main to client, we forward messages to external node.
            info!("From main to client");

...
impl<E, EM, M, S, Z> Stage<E, EM, S, Z> for TuneableMutationalStage<M>
where
    S: HasMetadata + HasCorpus + HasNamedMetadata + HasRand + HasExecutions + HasCurrentCorpusId,
    ...
    Z: Evaluator<E, EM, <S::Corpus as Corpus>::Input, S>,
{
    ...
    fn perform(
        ...
        let ret = self.do_mutations(fuzzer, executor, state, manager);
...
impl<M> TuneableMutationalStage<M> {
    ...
    #[must_use]
    pub fn new<S: HasNamedMetadata>(state: &mut S, mutator: M) -> Self {
    ...
    pub fn with_name<S: HasNamedMetadata>(state: &mut S, mutator: M, name: &str) -> Self {
    ...
    pub fn set_iters_std<S: HasNamedMetadata>(state: &mut S, iters: u64) -> Result<(), Error> {
        set_iters_by_name(state, iters, STD_TUNEABLE_MUTATIONAL_STAGE_NAME)
    }
    ...
    pub fn iters_std<S: HasNamedMetadata>(state: &S) -> Result<Option<u64>, Error> {
        get_iters_by_name(state, STD_TUNEABLE_MUTATIONAL_STAGE_NAME)
    }
    ...
    pub fn set_seed_fuzz_time_std<S: HasNamedMetadata>(
        state: &mut S,
        fuzz_time: Duration,
    ) -> Result<(), Error> {
        set_seed_fuzz_time_by_name(state, fuzz_time, STD_TUNEABLE_MUTATIONAL_STAGE_NAME)
    ...
    pub fn seed_fuzz_time_std<S: HasNamedMetadata>(
        &self,
        state: &S,
    ...
    pub fn reset_std<S: HasNamedMetadata>(state: &mut S) -> Result<(), Error> {
        reset_by_name(state, STD_TUNEABLE_MUTATIONAL_STAGE_NAME)
    }
    ...
    fn iterations<S: HasRand>(&self, state: &mut S) -> Result<usize, Error> {
        Ok(
            // fall back to random
    ...
    where
        S: HasCorpus + HasNamedMetadata + HasCurrentTestcase + HasRand + HasExecutions,
        M: Mutator<<S::Corpus as Corpus>::Input, S>,
        Z: Evaluator<E, EM, <S::Corpus as Corpus>::Input, S>,
        <S::Corpus as Corpus>::Input: Clone,
    {
        ...
        match (fuzz_time, iters) {
            (Some(fuzz_time), Some(iters)) => {
                ...
                for _ in 1..=iters {
                    ...
                    self.one_mutation(fuzzer, executor, state, manager, &input)?;
                }
            }
            (Some(fuzz_time), None) => {
            ...
            (None, None) => {
                // fall back to random
                let iters = self.iterations(state)?.saturating_sub(
                    self.restart_helper
...
impl<M> TuneableMutationalStage<M> {
    ...
    #[must_use]
    pub fn transforming<S: HasNamedMetadata>(state: &mut S, mutator: M, name: &str) -> Self {

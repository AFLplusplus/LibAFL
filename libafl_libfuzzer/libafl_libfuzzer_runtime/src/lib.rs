...
macro_rules! fuzz_with {
    ($options:ident, $harness:ident, $operation:expr, $and_then:expr, $edge_maker:expr) => {{
        ...
        let closure = |mut state: Option<_>, mut mgr, _cpu_id| {
            ...
            let std_power = StdPowerMutationalStage::new(std_mutator, "std power");
            ...
            let cm_power = StdPowerMutationalStage::new(custom_mutator, "cm power");
            ...
            let cc_std_power = StdPowerMutationalStage::new(std_mutator_no_crossover, "cc std power");
            let cc_std_power =

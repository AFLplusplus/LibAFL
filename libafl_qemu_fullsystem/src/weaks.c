...
__attribute__((weak)) void load_snapshot_restart(void) {
  
}
__attribute__((weak)) int libafl_qemu_remove_hook(uint64_t addr) {
  (void)addr;
  return 0;
}

__attribute__((weak)) int libafl_qemu_set_hook(uint64_t addr, void (*callback)(uint64_t), uint64_t value) {
  (void)addr;
  (void)callback;
  (void)value;
  return 0;

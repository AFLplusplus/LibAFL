...
`Linux` (through `perf`) and `Windows` (through the `ipt.sys` driver) hosts are supported.

To check if your host has all the features used by this crate, run `cargo test intel_pt_check_availability -- --show-output`
as `root` on `Linux` (`sudo -E cargo test ...`), or from an admin shell with the `ipt` service running (`sc start ipt`)
on `Windows`.

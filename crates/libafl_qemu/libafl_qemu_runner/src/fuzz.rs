...
pub struct FuzzerOptions {
    #[clap(short, long = "asan-host", help = "Enable Asan Host")]
    pub asan_host: bool,

    #[clap(
        short,
        long = "asan-guest",
        help = "Enable Asan Guest",
        conflicts_with = "asan_host"
    )]
    pub asan_guest: bool,
    ...
    #[arg(long = "asan-include", help="Include Asan address ranges", value_parser = FuzzerOptions::parse_ranges)]
    pub asan_include: Option<Vec<Range<GuestAddr>>>,

    #[arg(long = "asan-exclude", help="Exclude Asan address ranges", value_parser = FuzzerOptions::parse_ranges, conflicts_with="asan_include")]
    pub asan_exclude: Option<Vec<Range<GuestAddr>>>,
...
pub fn fuzz() {
    ...
    let asan_filter = if let Some(asan_include) = &options.asan_include {
        info!("ASAN includes: {asan_include:#x?}");
        StdAddressFilter::allow_list(asan_include.to_vec())
    } else if let Some(asan_exclude) = &options.asan_exclude {
        info!("ASAN excludes: {asan_exclude:#x?}");
        StdAddressFilter::deny_list(asan_exclude.to_vec())
    } else {

...
static FRONTEND: Lazy<Mutex<GuestFrontend>> = Lazy::new(|| {
    let _level = GuestEnv::initialize()
        .ok()
...
unsafe fn raw_write(fd: i32, buf: *const u8, count: usize) {
    unsafe {
        raw_syscall(1, fd as u64, buf as u64, count as u64, 0);
    }
}
...
unsafe fn log_to_file_raw(msg: &CStr) {
    ...
    let fd = unsafe { raw_syscall(2, path.as_ptr() as u64, 1089, 438, 0) };
    if fd as i64 > 0 {
        unsafe {
        ...
        unsafe { raw_syscall(1, 2, msg.as_ptr() as u64, msg.count_bytes() as u64, 0) };
    }
}
...
pub unsafe extern "C" fn asan_untrack(addr: *const c_void) {
    ...
    let _ = FRONTEND.lock().tracking_mut().untrack(addr as GuestAddr);
}
...
pub unsafe extern "C" fn realloc(ptr: *mut c_void, size: usize) -> *mut c_void {
    ...
    if !new_ptr.is_null() {
        ...
        unsafe {
            libc::memcpy(new_ptr, ptr, copy_size);
        }
        unsafe {
            asan_dealloc(ptr);
        }

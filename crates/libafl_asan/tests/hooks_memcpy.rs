...
mod tests {
    ...
    use libafl_asan::{expect_panic, hooks::memcpy::memcpy};

    #[test]
    fn test_memcpy_zero_length() {
        let ret = unsafe { memcpy(null_mut(), null_mut(), 0) };
        assert_eq!(ret, null_mut());
    }

    #[test]
    fn test_memcpy_src_null() {
        expect_panic();
        let data = [0u8; 10];
        unsafe { memcpy(null_mut(), data.as_ptr() as *const c_void, data.len()) };
        unreachable!();
    }

    #[test]
    fn test_memcpy_dst_null() {
        expect_panic();
        let data = [0u8; 10];
        unsafe { memcpy(data.as_ptr() as *mut c_void, null(), data.len()) };
        unreachable!();
    }
    ...
    fn test_adjacent_1() {
        ...
        let ret = unsafe { memcpy(dest, src, 10) };
        assert_eq!(ret, dest);
    }

...
fn main() {
    ...
    if !afl_path.is_dir(){
        ...
        Command::new("git").arg("clone").arg(AFL_URL).status().unwrap();
    }

    if !afl_gcc_path.is_file(){
        Command::new("make").current_dir(&afl_path).status().unwrap();
    }


    Command::new(afl_gcc_path).args(&["src/forkserver_test.c", "-o"])

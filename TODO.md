...
- [ ] Fix standard library import in `crates/ll_mp/src/lib.rs` line 135 to use `core` instead of `std` to resolve clippy error.
- [ ] Remove redundant reference `&` in `panic!` argument in `crates/ll_mp/src/lib.rs` around line 1835.
- [ ] Remove redundant reference `&` in `assert!` argument in `crates/ll_mp/src/lib.rs` around line 2087.
- [ ] Add unit tests for `ll_mp` specific edge cases around panic handling and asserts to ensure robustness.

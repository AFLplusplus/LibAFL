...
pub unsafe extern "C" fn asan_load(addr: *const c_void, size: usize) {
    let addr = addr as GuestAddr;
    trace!("load - addr: {addr:#x}, size: {size:#x}");
    if FRONTEND
        .lock()
        .shadow()
        ...
        .unwrap()
    {
        panic!("Poisoned - addr: {addr:#x}, size: {size:#x}");
    }
}
...
pub unsafe extern "C" fn asan_store(addr: *const c_void, size: usize) {
    let addr = addr as GuestAddr;
    trace!("store - addr: {addr:#x}, size: {size:#x}");
    if FRONTEND.lock().shadow().is_poison(addr, size).unwrap() {
        panic!("Poisoned - addr: {addr:#x}, size: {size:#x}");
    }
}

...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let instance_idx: usize = args[1].parse().map_err(|e| {
        format!("Failed to parse instance index '{}': {}", args[1], e)
    })?;

    let ci_instances: usize = match env::var("CI_INSTANCES") {
        Ok(val) => val.parse().map_err(|e| {
            format!("CI_INSTANCES must be a positive integer, got '{}': {}", val, e)
        })?,
        Err(_) => {
    ...
    let llvm_var: usize = match env::var("LLVM_VERSION") {
        Ok(val) => val.parse().map_err(|e| {
            format!("LLVM_VERSION must be a positive integer, got '{}': {}", val, e)
        })?,
        Err(_) => {

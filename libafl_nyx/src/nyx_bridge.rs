...
fn nyx_process_start(
    ...
    match NyxProcess::process_start(
        sharedir_r_str,
        workdir_r_str,
        worker_id,
        cpu_id,
...
pub fn nyx_get_input_buffer(nyx_process: &NyxProcess) -> *mut u8 {
    return nyx_process.input_buffer_mut().as_mut_ptr();
}

pub fn nyx_get_bitmap_buffer(nyx_process: &NyxProcess) -> *mut u8 {
...
pub fn nyx_shutdown(nyx_process: NyxProcess) {
    nyx_process.shutdown();
}

pub fn nyx_option_set_reload_mode(nyx_process: &mut NyxProcess, enable: bool) {
    nyx_process.option_set_reload_mode(enable);
}

pub fn nyx_option_set_timeout(nyx_process: &mut NyxProcess, timeout_sec: u8, timeout_usec: u32) {
    nyx_process.option_set_timeout(timeout_sec, timeout_usec);
}

pub fn nyx_option_apply(nyx_process: &mut NyxProcess) {
    nyx_process.option_apply();
}

pub fn nyx_exec(nyx_process: &mut NyxProcess) -> NyxReturnValue {
    nyx_process.exec()
}

pub fn nyx_set_afl_input(nyx_process: &mut NyxProcess, buffer: *mut u8, size: u32) {

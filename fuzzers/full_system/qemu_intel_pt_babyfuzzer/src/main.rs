//! An example fuzzer using qemu systemmode with KVM and Intel PT
...
use libafl::{
    ...
    mutators::{
        havoc_mutations::havoc_mutations, scheduled::HavocScheduledMutator, LoggerScheduledMutator,
    },
...
use libafl_qemu::{
    config,
    config::{Accelerator::Kvm, DriveCache, KvmProperties, QemuConfig},
    executor::QemuExecutor,
    modules::intel_pt::{AddrFilter, AddrFilterType, AddrFilters, IntelPTModule, SectionInfo},
...
const BOOTLOADER_CODE: RangeInclusive<u64> = 0x7c00..=0x7c00 + 0xB4;
const BOOTLOADER_SLEEP_FN_ADDR: GuestAddr = 0x7c76;
...
fn main() {
    ...
    let qemu_config = QemuConfig::builder()
        ...
        .cpu("host")
        .ram_size(config::RamSize::MB(4))
        .drives([config::Drive::builder()
            .format(config::DiskImageFileFormat::Qcow2)
            .file("target/boot.qcow2")
            .cache(DriveCache::None)
            .build()])
        .accelerator(Kvm(KvmProperties::default()))
        .default_devices(false)
        .bios("./target/debug/qemu-libafl-bridge/pc-bios")
        .start_cpu(false)
        .build();
    log::debug!("QEMU command: {qemu_config}");
    ...
    let image = [SectionInfo {
        filename,
        offset: 0,
        size: (BOOTLOADER_CODE.end() - BOOTLOADER_CODE.start()) + 1,
        virtual_address: *BOOTLOADER_CODE.start(),
    }];
    ...
    .unwrap();
    let intel_pt_builder = IntelPTModule::default_pt_builder().ip_filters(&filters);
    ...
    let mut harness = |emulator: &mut Emulator<_, _, _, (IntelPTModule<_>, ()), _, _, _>,
                       _state: &mut StdState<_, _, _, _>,
        ...
        let (pt_mod, _) = emulator.modules_mut().modules_mut();
        pt_mod.enable_tracing();
    ...
    let observer: StdMapObserver<'_, u8, false> =
    ...
    let mutator = LoggerScheduledMutator::new(HavocScheduledMutator::new(havoc_mutations()));
...
// try to implement a feedback that acepts Vec<InclusiveRange<u64>>

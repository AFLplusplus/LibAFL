...
impl<'a> Instance<'a> {
        ...
        let mut state = match state {
            ...
            None => {
                StdState::new(
                    ...
                    InMemoryOnDiskCorpus::no_meta(self.options.get_queue_dir(self.core_id))?,
                    // Corpus in which we store solutions (crashes in this example),
                    // on disk so the user can get them after stopping the fuzzer
                    OnDiskCorpus::new(self.options.get_crashes_dir(self.core_id))?,
    ...
    {
        let corpus_dirs = [self.options.get_input_dir()];

...
if [ ! -e mozjpeg-4.0.3/libjpeg.a ]; then
    cd mozjpeg-4.0.3
    cmake --disable-shared . -DCMAKE_C_COMPILER=$(realpath ../target/release/libafl_cc) -DCMAKE_CXX_COMPILER=$(realpath ../target/release/libafl_cxx) -G "Unix Makefiles" || exit 1
    make -j `nproc` || exit 1
    cd ..
fi

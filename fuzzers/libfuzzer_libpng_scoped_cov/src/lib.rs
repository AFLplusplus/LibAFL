...
#[cfg(feature = "crash")]
use std::ptr;
use std::{collections::BTreeMap, env, fs::File, io::stderr, os::fd::AsRawFd, path::PathBuf};
...
use libafl::{
    ...
    feedbacks::{
        CrashFeedback, MapFeedbackMetadata, MaxMapFeedback, TimeFeedback, TimeoutFeedback,
    },
    ...
    prelude::SimpleEventManager,
    schedulers::{
        powersched::PowerSchedule, IndexesLenTimeMinimizerScheduler, StdWeightedScheduler,
    },
    stages::{calibrate::CalibrationStage, power::StdPowerMutationalStage},
    state::{HasCorpus, HasMetadata, HasNamedMetadata, StdState},
    Error,
};
use libafl_bolts::{
    current_nanos,
...
fn fuzz(corpus_dirs: &[PathBuf], objective_dir: PathBuf, broker_port: u16) -> Result<(), Error> {
    ...
    //     };
    let state = None;
    let mut restarting_mgr = SimpleEventManager::printing();

    // Create an observation channel using the coverage map
    let edges_observer =
        unsafe { StdMapObserver::from_mut_ptr("edges", EDGES_MAP.as_mut_ptr(), MAX_EDGES_NUM) };
    ...
    let history_map = &state
        .named_metadata::<MapFeedbackMetadata<u8>>("mapfeedback_metadata_edges")
        .unwrap()
        .history_map;

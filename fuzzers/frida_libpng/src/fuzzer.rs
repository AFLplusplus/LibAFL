...
use frida_gum::{Gum, CpuContext, NativePointer, MemoryRange};
            ...
            for instruction in basic_block {
                if first {
                    instruction.put_callout(|cpu_context| {
                        ...
                        //println!("on_basic_block: {:x}", current_pc);
                        current_pc = (current_pc >> 4) ^ (current_pc << 8);

                        current_pc &= (1024 * 1024) - 1;
...
fn fuzz(corpus_dirs: Vec<PathBuf>, objective_dir: PathBuf, broker_port: u16) -> Result<(), Error> {
    ...
    let mut frida_helper = FridaHelper::new();



    // Create an observation channel using the coverage map
    let edges_observer = HitcountsMapObserver::new(unsafe {
        StdMapObserver::new_from_ptr("edges", frida_helper.map.as_mut_ptr(), 1024*1024)
    });

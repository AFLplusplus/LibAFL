// jif.cc
// Heavily based off headless_example.cc from chromium project:
// https://chromium.googlesource.com/chromium/src/+/master/headless/app/headless_example.cc
...
void JIF::OnLoadEventFired(const headless::page::LoadEventFiredParams &params) {
  if (VERBOSE_MODE) {
    printf("Loaded. DOM:\n");
    ...
    devtools_client_->GetRuntime()->Evaluate(
        ...
        base::BindOnce(&JIF::OnDomFetched, weak_factory_.GetWeakPtr()));
  }

  page_loaded_cv.notify_all();
}

void JIF::OnJavascriptDialogOpening(
...
// convert a string to a data:text/html url where the input string is the base64
// encoded content of the html file
...
// This function is called by the harness to load a javascript function into the
// page using the Evaluate command.
...
void OnPreciseCoverageStarted(
    std::unique_ptr<headless::profiler::StartPreciseCoverageResult> result) {
  precise_coverage_started_cv.notify_all();
}

extern "C" const char *get_js_coverage() {
  if (js_coverage != NULL) return (const char *)js_coverage;
  return "";
}

void OnPreciseCoverageCompleted(
    std::unique_ptr<headless::profiler::TakePreciseCoverageResult> result) {
  if (result) {
    ...
    auto buffer = (char *)malloc(
        json.length() +
        1);  // please do not provide a coverage string of length INT_MAX
    CHECK(buffer);
...
std::string RunTestCaseThroughHarness(std::string test_case) {
  ...
  for (int i = 0; i < test_case.length(); i++) {
    if (isprint(test_case[i])) { ascii_test_case.push_back(test_case[i]); }
  }
  ...
  jif_global->devtools_client_js->GetRuntime()->Evaluate(
      payload, base::BindOnce(base::BindOnce(&OnHarnessFinished)));
  ...
  if (processed_test_case == "") {
    if (VERBOSE_MODE) {
      printf(
          "Harness returned the empty string. The triggering input was: %s\n",
  ...
  // BUG: if we stop precise coverage, all future calls to start precise
  // coverage fail so we never do!
...
extern "C" int LLVMFuzzerInitialize(int *argc, char ***argv) {
  ...
  headless::HeadlessBrowser::Options::Builder builder(*argc,
                                                      (const char **)*argv);
  ...
  std::thread headless_browser_main_thread(
  ...
  // wait on the condition variable headless_startup_cv until the
  // headless_browser_main_thread is ready and has set jif_global to a valid JIF
  // instance
  ...
  task_runner = content::GetUIThreadTaskRunner(
      {});  // doesnt seem to make a perf difference whether we use UI or IO
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  if (VERBOSE_MODE) printf("Pre-Harness: %s\n", data);

  // check length here

  // run the test case through the harness
  std::string result = RunTestCaseThroughHarness(
      std::string(reinterpret_cast<const char *>(data), size));

  if (VERBOSE_MODE) printf("Post-Harness: %s\n", result.c_str());
  ...
  if (url.length() > MAX_TESTCASE_LENGTH) {
    if (VERBOSE_MODE)
      printf("Testcase too long skipping. Encoded length %lu. Original: %s\n",
             url.length(), data);
    return 0;
  }
  // load the url on the task runner

  task_runner->PostTask(FROM_HERE, base::BindOnce(&LoadURL, url));

/*!
libafl_wizard is a crate that allows users to generate fuzzers that make use of
Libafl's components by simply answering some questions.

This is the entry point into libafl_wizard.
*/
...
use utils::{
    arrange_code, arrange_imports, clear_terminal_screen, separate_imports, validate_input,
    write_code,
};
...
fn main() {
    ...
    while questions[curr_q].id() != "END" {
        ...
        if validate_input(&input, &String::from("Undo")) {
            ...
            let answers = questions[next_q].answers();

            if !answers[ans_i].skip().is_empty() {
                questions[next_q]
                    .clone()
            ...
            // If there are any questions that should be skipped because of that answer, we skip them.
            if !answers[ans_i].skip().is_empty() {
                questions[curr_q]
                    .clone()
    ...
    println!("IMPORTS:");
    for i in &imports_content {
        println!("{}", i);
    }

    // Separate by instances of components/imports, arrange them in the correct order and write to the file.
    let file_name = write_code(arrange_code(code_content), arrange_imports(imports_content));
